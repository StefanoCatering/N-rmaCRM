...
- **Migraciones SQL**: se aplican manualmente en el SQL Editor de Supabase (patrón idempotente: DROP constraint viejo si no incluye el valor nuevo, ADD el nuevo). El usuario las ejecuta; Claude no ejecuta DDL en prod. Pendientes de correr en Supabase a la fecha: `migrations/fase4_fix_monto_pagado.sql`, `migrations/fase4_drop_legacy_tables.sql`, `migrations/fase5_indices.sql`.
